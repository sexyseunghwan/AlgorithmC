...
int table[3][101];//내용을 모두 0 으로 초기화 시켜준다.

int main_2979()
{
	int one_fee, two_fee, three_fee;

	int total_fee = 0;

	scanf("%d %d %d", &one_fee, &two_fee, &three_fee);
	...
	for (int i = 1; i < 101; i++) {
		int count = table[0][i] + table[1][i] + table[2][i];
